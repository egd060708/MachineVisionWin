...
void Palm::spilt_palm_vein()
{

	// 滚动条二值化
	//  namedWindow("THRESH_BINARY", WINDOW_AUTOSIZE);
	//  temp_binary = this->palm_vein_enhance.clone();
	...
	cv::threshold(this->palm_vein_enhance, split_temp, 49, 255, THRESH_BINARY); // OTSU的效果不如手调的阈值
	...
	this->palm_vein_split = split_temp.clone();
	imshow("vein_split", palm_vein_split);
	waitKey(1);
}
