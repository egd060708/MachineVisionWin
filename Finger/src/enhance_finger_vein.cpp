...
int clioLimit = 9000; // 32bit图 大一点
int size_1 = 3;		  // size 设置成五恰好吧
int size_2 = 28;	  // cliomit高的时候 ，size2需要设置的小一些 50下降到45吧 //调整到27可以保留横向和纵向的静脉
Mat temp_CLAHE;
 ...
 * @brief 静脉纹理增强
 */
void Finger::enhance_finger_vein()
{
	this->finger_vein_enhance = this->finger_roi.clone(); // 复制

	Mat temp16bit;
	cv::cvtColor(this->finger_vein_enhance, temp16bit, COLOR_BGR2GRAY);
	temp16bit.convertTo(temp16bit, CV_16U, 256, 0); // 将roi图转换成16位 提高精度
	...
	//  on_Trackbar_CLAHE(size_1, 0);
	//  on_Trackbar_CLAHE(clioLimit, 0);
	//  on_Trackbar_CLAHE(size_2, 0);

	// CLAHE算法进行直方图均衡化
	auto cla = cv::createCLAHE(9000, Size(3, 28));
	...
	// 转32F归一化
	cv::Mat temp32bit_fl;
	temp16bit.convertTo(temp32bit_fl, CV_32F, 1.0 / (256 * 256 - 1));
	...
	// 利用滤波函数处理32bit图像，并最后生成8bit图 存在finger_vein_enhance中
	{
		...
		frangi2d(temp32bit_fl, vesselness, scale, angles, opts);	 // frangi滤波
		// 显示结果
		vesselness.convertTo(this->finger_vein_enhance, CV_8UC1, 255);
		//  scale.convertTo(scale, CV_8UC1, 255);
		//  angles.convertTo(angles, CV_8UC1, 255);
		// imshow("finger_vein_enhance", finger_vein_enhance);
		// waitKey(1);
	}

	// TODO 是否需要使用grao函数增强一次呢？
}
