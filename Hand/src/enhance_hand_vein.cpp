 ...
 * @brief 静脉纹理增强
 */
void Hand::enhance_hand_vein()
{
	this->hand_vein_enhance = this->hand_roi.clone(); // 复制

	// finger从bgr转换成灰度图，再从8bit转换成16bit，提高精度，让静脉增强效果更好，参数更细致
	Mat temp16bit;
	cv::cvtColor(this->hand_vein_enhance, temp16bit, COLOR_BGR2GRAY);
	...
	// 利用滤波函数处理32bit图像，并最后生成8bit图，存在finger_vein_enhance中
	{
		...
		vesselness.convertTo(this->hand_vein_enhance, CV_8UC1, 255);
		// scale.convertTo(scale, CV_8UC1, 255);
		// angles.convertTo(angles, CV_8UC1, 255);
		imshow("finger_vein_enhance", this->hand_vein_enhance);
		waitKey(1);
	}
}
