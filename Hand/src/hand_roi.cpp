...
// int scale_int =1 ;
// Mat temp_hand_cut ;
// void on_Trackbar_hand_cut(int, void *) {
//  Mat temp =temp_hand_cut.clone() ;
//  double temp_scale = double(scale_int)/100 ;
//  int row_end_temp = (temp.rows-hand_center.y)*cut_scale+hand_center.y-1 ;
//  temp_roi.rowRange(0 , row_end);
//  imshow("temp_roi",temp_roi) ;
//  waitKey(0) ;
//  Mat temp=temp_hand_cut.rowRange(0 , );
//  cv::threshold(temp_hand_cut, temp, thresh_threshold, 255, THRESH_BINARY);
//  {
//    cout << "thresh_threshold_split" << thresh_threshold << endl;
//    imshow("THRESH_BINARY", temp);
//
...
void Hand::hand_roi_process()
{
	...
	// 转灰度图
	Mat temp_roi;
	cvtColor(this->hand_src, temp_roi, COLOR_BGR2GRAY);
	...
	// 寻找掌心，认为距离轮廓最远的点为掌心
	{
		...
		for (int i = 0; i < temp_roi.cols; i++)
		{
			for (int j = 0; j < temp_roi.rows; j++)
			{
				...
				{
					maxdist = dist;
					hand_center = Point(i, j);
				}
			}
		}
		Mat temp = temp_roi.clone();
		circle(temp, hand_center, maxdist, Scalar(0));
		imshow("CIRCLE", temp);
		waitKey(1);
	}

	//   temp_hand_cut=temp_roi.clone() ;
	//  namedWindow("HandCut",WINDOW_AUTOSIZE) ;
	//   createTrackbar("scale", "HandCut", &scale_int  , 255, on_Trackbar_hand_cut);
	//   on_Trackbar_hand_cut(0, 0);

	// 手腕剔除
	{
		double cut_scale = 0.8;
		int row_end = maxdist * cut_scale + hand_center.y;
		temp_roi = temp_roi.rowRange(0, row_end).clone();
		// 定义结构元素
		cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(7, 7));
		// 开运算
		cv::morphologyEx(temp_roi, temp_roi, cv::MORPH_OPEN, kernel);
		// imshow("temp_handcut", temp_roi);
		// waitKey(0);
	}
	...
	// 从原图中提取ROI
	{
		...
		// 进行仿射变换
		Mat temp_img;
		cv::warpAffine(hand_src, temp_img, rotationMatrix, hand_src.size());
		...
		Rect hand_roi_rect(rect_org_point.x, rect_org_point.y, width, height);

		this->hand_roi = temp_img(hand_roi_rect).clone();
		resize(hand_roi, hand_roi, Size(240, 240));
		imshow("hand_roi_rect", this->hand_roi);
		waitKey(1);
	}
}
